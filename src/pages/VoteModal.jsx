...
import {
  ...
  Button, Avatar, Typography, Box, Alert, CircularProgress,
  IconButton, Grid, Divider, Chip,
} from '@mui/material';
import {
  Close as CloseIcon,
...
import axiosInstance from '../api/axios'; // ✅ import unifié
...
const VoteModal = ({ open, onClose, candidat, edition }) => {
  ...
  const handleVote = async () => {
    try {
      ...
      // ✅ Plus besoin de passer Authorization manuellement — l'intercepteur s'en charge
    ...
    <Dialog open={open} onClose={() => onClose(false)} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ m: 0, p: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Typography variant="h6">Confirmer votre vote</Typography>
          <IconButton onClick={() => onClose(false)} sx={{ color: 'grey.500' }}>
            <CloseIcon />
          </IconButton>
        </Box>
      </DialogTitle>
        ...
        {success ? (
          ...
          <>
            ...
            <Box sx={{ textAlign: 'center', mb: 3 }}>
              <Avatar
                src={candidat.photo_url || candidat.photo}
              ...
              {candidat.ethnie && <Chip label={candidat.ethnie} sx={{ mb: 2 }} />}
            </Box>
            ...
            <Grid container spacing={2}>
              <Grid item xs={6}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Typography variant="body2">{candidat.universite}</Typography>
                </Box>
              </Grid>
              <Grid item xs={6}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <GroupsIcon color="action" />
                  <Typography variant="body2">{candidat.entite}</Typography>
                </Box>
              </Grid>
              {candidat.filiere && (
                <Grid item xs={12}>
      ...
      <DialogActions sx={{ p: 2 }}>
        {!success && (
          <>
            <Button onClick={() => onClose(false)} disabled={loading}>Annuler</Button>
            <Button
              variant="contained"
              onClick={handleVote}
              disabled={loading}
              ...
              sx={{
                ...
                color: 'white',
                '&:hover': { background: 'linear-gradient(45deg, #c19b30 30%, #e68a2e 90%)' },
              }}
            >
